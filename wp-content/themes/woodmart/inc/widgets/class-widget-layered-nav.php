	...
	class WOODMART_Widget_Layered_Nav extends WPH_Widget {
		...
		protected function layered_nav_dropdown( $terms, $taxonomy, $query_type ) {
			...
			if ( $taxonomy !== $this->get_current_taxonomy() ) {
				...
				echo '<form method="get" action="' . esc_url( $form_action ) . '" class="woodmart-widget-layered-nav-dropdown-form">';
				echo '<select class="woodmart-widget-layered-nav-dropdown woodmart_dropdown_layered_nav_' . esc_attr( $taxonomy_filter_name ) . '"' . ( $multiple ? 'multiple="multiple"' : '' ) . ' data-placeholder="' . esc_attr( $any_label ) . '" data-noResults="' . esc_html__( 'No matches found', 'woodmart' ) . '" data-slug="' . esc_attr( $taxonomy_filter_name ) . '">';
				...
				if ( $multiple ) {
					echo '<button class="woodmart-widget-layered-nav-dropdown__submit" type="submit" value="' . esc_attr__( 'Apply', 'woodmart' ) . '">' . esc_html__( 'Apply', 'woodmart' ) . '</button>';
				}
		...
		protected function layered_nav_list( $terms, $taxonomy, $query_type, $instance ) {
			...
			foreach ( $terms as $term ) {
				...
				echo ( true == $option_is_set || $count > 0 ) ? '<a rel="nofollow" href="' . esc_url( apply_filters( 'woocommerce_layered_nav_link', $link ) ) . '" class="layered-nav-link">' : '<span>';
		...
		protected function layered_nav_checkbox_list( $terms, $taxonomy, $query_type, $instance ) {
			...
			echo '<div class="woodmart-pf-checkboxes woodmart-pf-attributes' . esc_attr( $multi_select ) . '">';
				...
				echo '<div class="woodmart-pf-title"><span class="title-text">' . esc_html( $title ) . '</span><ul class="woodmart-pf-results"></ul></div>';
				echo '<div class="woodmart-pf-dropdown woodmart-scroll">';
					echo '<ul class="woodmart-scroll-content' . esc_attr( $classes ) . '">';
						foreach ( $terms as $term ) {
							$current_values = isset( $_chosen_attributes[ $taxonomy ]['terms'] ) ? $_chosen_attributes[ $taxonomy ]['terms'] : array();
							$option_is_set = in_array( $term->slug, $current_values );
							...
							if( ! empty( $swatch_style ) ) {
								$swatch_div = '<span style="' . $swatch_style. '" class="' . ( ( $labels == 'off' ) ? 'woodmart-tooltip' : '' ) . '">' . esc_html( $term->name ) . '</span>';
							}
							// END swatches customization

							echo '<li class="woodmart-pf-' . esc_attr( $term->slug ) . esc_attr( $class ) . '">';
