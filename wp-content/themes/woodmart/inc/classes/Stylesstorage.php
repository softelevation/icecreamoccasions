...
class WOODMART_Stylesstorage {
	...
	public function check_css_status() {
		...
		if ( version_compare( $this->current_theme_version, $this->css_version, '==' ) && 'valid' === $data_status && md5( $this->current_site_url ) === $this->site_url ) {
			if ( isset( $this->data['path'] ) && file_exists( $this->get_file_path( $this->data['path'] ) ) && apply_filters( 'woodmart_styles_storage_file', true ) ) {
