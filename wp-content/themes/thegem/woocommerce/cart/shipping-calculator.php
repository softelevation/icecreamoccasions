...
<form class="woocommerce-shipping-calculator shipping_calculator" action="<?php echo esc_url( wc_get_cart_url() ); ?>" method="post">
	...
	<h2><span class="light"><?php echo esc_html( ! empty( $button_text ) ? $button_text : esc_html__( 'Calculate shipping', 'woocommerce' )); ?></span></h2>
	...
	<section class="shipping-calculator-form clearfix">
		...
		<p class="form-row form-row-wide full" id="calc_shipping_state_field">
			<?php
		...
		<?php if ( apply_filters( 'woocommerce_shipping_calculator_enable_city', true ) ) : ?>
			<p class="form-row form-row-wide full" id="calc_shipping_city_field">
				<input type="text" class="input-text" value="<?php echo esc_attr( WC()->customer->get_shipping_city() ); ?>" placeholder="<?php esc_attr_e( 'City', 'woocommerce' ); ?>" name="calc_shipping_city" id="calc_shipping_city" />
			</p>
		<?php endif; ?>

		<?php if ( apply_filters( 'woocommerce_shipping_calculator_enable_postcode', true ) ) : ?>
			<p class="form-row form-row-wide full" id="calc_shipping_postcode_field">
		...
		<?php
			thegem_button(array(
				'tag' => 'button',
				'text' => esc_html__( 'Update totals', 'thegem' ),
				'style' => 'outline',
				'size' => 'medium',
				'attributes' => array(
					'name' => 'calc_shipping',
