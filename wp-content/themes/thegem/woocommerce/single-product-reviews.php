		...
		<div id="review_form_wrapper">
			<div id="review_form">
				<?php
					...
					$comment_form = array(
						'title_reply'          => __( 'Add <span class="light">a review</span>', 'thegem' ),
						'title_reply_to'       => __( 'Leave a Reply to %s', 'woocommerce' ),
						'comment_notes_before' => '',
						'comment_notes_after'  => '',
						'fields'               => array(
							'author' => '<div class="col-md-6 col-xs-12 comment-author-input"><input type="text" name="author" id="comment-author" value="'.esc_attr($comment_author).'" size="22" tabindex="1" required placeholder="'.esc_attr__('Name', 'thegem').($req ? ' *' : '').'" /></div>',
							'email' => '<div class="col-md-6 col-xs-12 comment-email-input"><input type="text" name="email" id="comment-email" value="'.esc_attr($comment_author_email).'" size="22" tabindex="2" required placeholder="'.esc_attr__('Mail', 'thegem').($req ? ' *' : '').'" /></div>',
						),
					...
					$comment_form['comment_field'] .= '<p><label for="comment">'.esc_html__('Your message', 'thegem').'</label><textarea name="comment" id="comment" cols="58" rows="10" tabindex="4" required></textarea></p>';
