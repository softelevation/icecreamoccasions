...
function thegem_setup() {
	...
	if(!get_option('wpb_js_content_types')) {
		update_option('wpb_js_content_types', array('post', 'page', 'product', 'thegem_news', 'thegem_pf_item', 'thegem_footer', 'thegem_title'));
	}
...
function thegem_admin_menu_additional_links() {
	global $submenu;
	$submenu['thegem-theme-options'][] = array(esc_html__('Support Center', 'thegem'), 'edit_theme_options', esc_url('https://codexthemes.ticksy.com/'));
	$submenu['thegem-theme-options'][] = array(esc_html__('Documentation', 'thegem'), 'edit_theme_options', esc_url('http://codex-themes.com/thegem/documentation/'));
}
...
function thegem_scripts() {
	...
	wp_register_style('thegem-quickfinders', get_template_directory_uri() . '/css/thegem-quickfinders.css');
	wp_register_style('thegem-quickfinders-vertical', get_template_directory_uri() . '/css/thegem-quickfinders-vertical.css');
	...
	$id = is_singular() ? get_the_ID() : 0;
	$title_params = thegem_get_sanitize_page_title_data($id);
	if(is_tax() || is_category() || is_tag()) {
		$thegem_term_id = get_queried_object()->term_id;
		$title_params = thegem_theme_options_get_page_settings('blog');
		if(get_term_meta($thegem_term_id , 'thegem_taxonomy_custom_page_options', true)) {
			$title_params = thegem_get_sanitize_page_title_data($thegem_term_id, array(), 'term');
		}
	}
...
function thegem_admin_scripts_init() {
	...
	wp_enqueue_style('thegem-admin-styles', get_template_directory_uri() . '/css/thegem-admin.css', array(), '1.2');
	...
	wp_enqueue_script('thegem-admin-functions', get_template_directory_uri() . '/js/thegem-admin_functions.js', array('jquery'), '1.1');
...
function thegem_vc_frontend_editor_enqueue_js_css() {
	...
	wp_enqueue_script('thegem-vc-editor-init', get_template_directory_uri() . '/js/thegem-vc-editor-init.js', array(), false, true );
}
...
if(!function_exists('thegem_page_title')) {
	function thegem_page_title() {
		...
		$alignment = 'center';
		if (is_singular() || is_tax() || is_category() || is_tag() || is_search() || is_archive() || is_post_type_archive('product') || is_tax('product_cat') || is_tax('product_tag') || (is_404() && get_post(thegem_get_option('404_page')))) {
			$post_id = 0;
			...
			if ($page_data['title_background_image']) {
				if ($parallax_bg) {
					$parallax_bg_style = 'background-image: url(' . $page_data['title_background_image'] . ');';
				} else {
		...
		$output .= '<div class="page-title-title" style="' . esc_attr($css_title_margin) . '">' . ($rich_title ? $rich_title : '<h1 style="' . esc_attr($css_style_title) . '"' . ($xlarge ? 'class="title-xlarge"' : '') . '>' . thegem_title('', false) . '</' . ($title_style == '2' ? 'h2' : 'h1') . '>') . '</div>';
		if ($excerpt) {
		...
		if($title_style == 2 && !empty($page_data['title_template']) && get_post($page_data['title_template']) && !(defined('WPB_VC_VERSION') && (vc_is_frontend_editor() || vc_is_page_editable()))) {
			...
			return '<div id="page-title" class="page-title-block page-title-alignment-' . esc_attr($alignment) . ' page-title-style-' . esc_attr($title_style) . ' ' . esc_attr($title_class) . ($parallax_bg ? ' page-title-parallax-background-wrap' : '') . '" style="' . esc_attr($css_style) . '">
...
function thegem_fonts_manager_add_page() {
	$page = add_submenu_page('thegem-theme-options', esc_html__('Self-Hosted Fonts','thegem'), esc_html__('Self-Hosted Fonts','thegem'), 'edit_theme_options', 'fonts-manager', 'thegem_fonts_manager_page');
...
function thegem_fonts_manager_page(){
	...
	$thegem_get_theme_options = thegem_get_theme_options();
	foreach ($thegem_get_theme_options['fonts']['subcats'] as $key=>$item) {
		$fallback_fonts_elements_list[$key]['title'] = $item['title'];
		$font_only = true;
		foreach ($item['options'] as $k=>$option) {
			if (preg_match('/font_size/', $k)) {
				$fallback_fonts_elements_list[$key]['font_size'] = thegem_get_option($k);
				$font_only = false;
			}

			if (preg_match('/line_height/', $k)) {
				$fallback_fonts_elements_list[$key]['line_height'] = thegem_get_option($k);
				$font_only = false;
			}
...
function thegem_body_class($classes) {
	...
	if ($effects_params['effects_page_scroller']) {
		if ($effects_params['effects_page_scroller_type'] == 'basic') {
            ...
            if ($effects_params['fullpage_fixed_background']) {
